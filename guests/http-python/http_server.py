...
import asyncio
import hashlib
import poll_loop # Example unsing imports from another file

from rvm import exports
from rvm.types import Ok
from rvm.imports import host
from rvm.imports.host import (
    client_secret,
)
from rvm.imports import types
from rvm.imports.types import (
    Method_Get,
    Method_Post,
    Scheme,
    Scheme_Http,
...
async def handle_async(
    ...
    elif isinstance(method, Method_Get) and path == "/secret":
        response = OutgoingResponse(Fields.from_list([("content-type", b"text/plain")]))
        response_body = response.body()
        ResponseOutparam.set(response_out, Ok(response))
        sink = Sink(response_body)
        # Get some secret from the host
        secret = client_secret()
        await sink.send(bytes(secret, "utf-8"))
        sink.close()
